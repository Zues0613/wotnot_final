<template>
  <div class="p-4 sm:p-6">
    <div class="flex flex-col items-start justify-between gap-4 pb-5 mb-4 border-b md:flex-row md:items-center">
      <div>
        <h2 class="text-xl font-bold text-green-800 md:text-2xl">Broadcast Messages</h2>
    ...
    <PopUp v-if="showPopup" @close="closePopup()"
      ...
      <h2 class="text-xl font-semibold text-green-800 mb-4">New Broadcast</h2>

      <hr class="pb-4">


      <div class="flex flex-col gap-6 lg:flex-row">
        <div class="w-full lg:flex-1 max-h-[80vh] overflow-y-auto custom-scrollbar p-1">
          <form @submit.prevent="handleBroadcast" id="messageForm"
            ...
            <p class="mb-2 text-sm ">Add broadcast name and template below</p>
            <div class="p-4 bg-[#f5f6fa] mb-4 rounded-lg">
              <div class="mb-4">
              ...
              <div class="mb-2 ">
                ...
                <select id="template-select" v-model="selectedTemplateId" @change="onTemplateSelect"
                  class="w-full px-3 py-2 border border-gray-300 rounded max-h-[50px]" required>
                ...
                <div v-if="selectedTemplateHasMedia" class="mt-4">
                  <label for="" class="block text-sm font-semibold">Upload Media</label>
                  <input type="file" @change="onFileChange" class="w-full mb-2 mr-1 text-sm">
                  <div v-if="uploadedMedia">
                    <p class="font-bold text-green-800">Uploaded</p>
                  </div>
                </div>

                <div v-if="selectedTemplateHasParameters" class="mt-4">
                  <label for="">Select Parameter</label>
                  <select name="" id="" v-model="bodyParameter" class="w-full px-3 py-2 border border-gray-300 rounded">
            ...
            <p class="mb-2 text-sm ">Select contacts below or <a
                href="https://docs.google.com/spreadsheets/d/1aZ3t1j4nL_f8a-y4O5e5Z3f3e5Z_1Z_8/edit?usp=sharing&ouid=112687474452332634945&rtpof=true&sd=true"
            ...
            <div class="p-4 bg-[#f5f6fa] mb-4 rounded-lg">
              ...
              <div class="mb-1">
                ...
                <input type="file" @change="handleFileUpload" class="w-full mb-2 mr-1 text-sm" accept=".csv" />
              </div>
            </div>
            <div v-if="csvUploaded">
              <h4><b>Imported Contacts</b></h4>
              <p class="mb-2 text-sm ">Imported contacts are not saved to your contacts directory</p>
            </div>

            <div v-else>
              <h4 class="text-green-700"><b>Contacts</b></h4>
              <p class="mb-2 text-sm ">Select from your saved contacts</p>
            </div>

            <div class="flex flex-col sm:flex-row bg-[#f5f6fa] items-stretch sm:items-center gap-2 p-2 justify-between rounded-lg">
              <h3 class="text-l"><b>Filter by tags:</b></h3>
              <input type="text" v-model="tag_key" placeholder="Key"
                class="w-full px-3 py-2 border border-gray-300 rounded sm:w-auto">
              <input type="text" v-model="tag_value" placeholder="Value"
                class="w-full px-3 py-2 border border-gray-300 rounded sm:w-auto">
              <button @click.prevent="fiterBytTags"
                class="relative w-full p-2 my-2 text-sm text-gray-800 bg-gray-300 rounded sm:w-auto hover:bg-gray-400">Apply
                Filter</button>
            </div>

            <div class="overflow-x-auto max-h-[20vh] mb-8 custom-scrollbar mt-4">
              <table class="w-full border-collapse contact-table rounded-lg">
                <thead>
                  <tr class="bg-[#ffffff] text-center">
                    <th class="sticky top-0 z-10 p-2 bg-[#ffffff]">
                      <input type="checkbox" @change="selectAll($event)" v-model="allSelected"
                        class="z-10 m-2 scale-150">Select
                    </th>
                    <th class="sticky top-0 p-2 bg-[#ffffff]">Name</th>
                    <th class="sticky top-0 p-2 bg-[#ffffff]">Phone Number</th>
                    <th class="sticky top-0 p-2 bg-[#ffffff]">Tags</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="contact in contacts" :key="contact.id">
                    <td class="p-2 text-center scale-125 md:p-4">
                      <input type="checkbox" v-model="selectedContacts" :value="`${contact.name}:${contact.phone}`">
                    </td>

                    <td class="p-2 text-center md:p-4">{{ contact.name }}</td>
                    <td class="p-2 text-center md:p-4">{{ contact.phone }}</td>
            ...
            <p class="mb-2 text-sm ">Select from the options below</p>

            <div class="p-4 bg-[#f5f6fa] mb-4 rounded-lg">
              <p class="mb-1 text-sm font-semibold"><input type="radio" v-model="isScheduled" :value="false"
                  class="m-2 text-green-500 scale-150 accent-green-700">Send Now</p>
              <p class="mb-1 text-sm font-semibold"><input type="radio" v-model="isScheduled" :value="true"
                  class="m-2 text-green-500 scale-150 accent-green-700" @click="currentDateTime">Schedule</p>

              <div v-if="isScheduled" class="flex flex-col justify-between gap-4 sm:flex-row">
                <div class="flex-1">
            ...
            <button type="submit"
              class="flex items-center justify-center px-6 py-3 font-medium text-white bg-green-700 rounded-lg shadow-lg "
              ...
              {{ isSubmitted ? "Submitted" : popupLoading ? "Submitting..." : isScheduled ? 'Schedule Message' : 'Send Message' }}
            </button>
          </form>
        </div>

        <div
          class="flex-col flex-grow hidden h-full overflow-y-auto aspect-[10/19] min-w-[320px] p-3 max-h-[600px] bg-[url('@/assets/chat-bg.jpg')] bg-cover bg-center custom-scrollbar lg:flex rounded-lg shadow-inner">
          <div class="message">
    ...
    <div class="p-4 space-x-2 filter-container">
      <div class="flex items-center">
        <h3 class="mb-2 text-xl text-green-700 md:text-2xs"><b>Broadcast List </b></h3>
        <div class="pb-2 pl-2">
          <button class="text-green-600 underline hover:bg-transparent hover:text-green-700"
            @click="fetchBroadcastList(this.filterStatus, 1)">
            <i class="inline-block text-xl bi bi-arrow-clockwise" :class="{ 'animate-spin': loading }"></i>
          </button>
        </div>
      </div>
      ...
      <div v-if="broadcasts.length > 0" class="flex justify-between">
        <div class="flex items-center space-x-2 filter-container">
          <h3><b>Filter by:</b></h3>
          <div class="w-40px">
            <select name="" id="" @change="filterBroadcastsByStatus" class="px-3 py-2 border border-gray-300 rounded ">
    ...
    <div v-if="broadcasts.length > 0" class="overflow-x-auto max-h-[51vh] custom-scrollbar">
      <table class="w-full text-sm bg-white border border-gray-300 rounded-lg md:text-base">
        <thead>
          <tr class="font-semibold text-center text-gray-700 bg-gray-100">
            <th class="sticky top-0 z-10 p-3 text-left bg-gray-100 border border-gray-300 md:p-4">Broadcast Name</th>
            <th class="sticky top-0 z-10 p-3 border border-gray-300 bg-gray-100 md:p-4">Type</th>
            <th class="sticky top-0 z-10 p-3 border border-gray-300 bg-gray-100 md:p-4">Template</th>
            <th class="sticky top-0 z-10 p-3 border border-gray-300 bg-gray-100 md:p-4">Contacts</th>
            <th class="sticky top-0 z-10 p-3 border border-gray-300 bg-gray-100 md:p-4">Status</th>
            <th class="sticky top-0 z-10 p-3 border border-gray-300 bg-gray-100 md:p-4">Action</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="broadcast in broadcasts" :key="broadcast.id" class="hover:bg-gray-50">
            <td class="p-3 text-left border border-gray-200 md:p-4">{{ broadcast.name }}</td>
            <td class="p-3 text-center border border-gray-200 md:p-4">{{ broadcast.type }}</td>
            ...
            <td class="p-3 text-center border border-gray-200 md:p-4">
              <div :class="{
                'text-emerald-600 font-semibold': broadcast.status === 'Successful',
                'text-sky-600 font-semibold': broadcast.status === 'Scheduled',
                'text-rose-600 font-semibold': broadcast.status === 'Cancelled',
                'text-amber-600 font-semibold': broadcast.status === 'Partially Successful',
            ...
            <td class="p-3 text-center border border-gray-200 md:p-4">
              <button class="font-medium text-gray-800 underline hover:bg-inherit"
      ...
      <div class="flex items-center px-4 py-2 space-x-4 bg-white rounded-lg shadow-md">
        ...
        <button class="px-3 py-1 font-medium text-white bg-green-500 rounded hover:bg-green-600"
          @click="loadNextPage">
          Next
        </button>
      </div>
    ...
    <PopUp1 v-if="showReportPopup" @close="showReportPopup = false, broadcastReports = ['']">
      <h3 class="mb-4 text-xl md:text-2xs"><b>Broadcast Report</b></h3>
      <div class="grid grid-cols-1 gap-4 p-4 sm:grid-cols-2 xl:grid-cols-5">
        <!-- Sent -->
        <div class="flex flex-col p-4 border-2 border-green-600 rounded-md shadow-sm bg-gray-100 items-left">
          <div class="flex items-center justify-between">
            <span class="text-3xl font-semibold">{{ totals.sent }}</span>
            <div class="flex items-center justify-center w-10 h-10 mb-auto bg-white rounded-full"><svg width="18"
                height="14" viewBox="0 0 18 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M1.66797 7L7.16797 12.5L16.3346 1.5" stroke="#23A455" stroke-width="2"
        ...
        <div class="flex flex-col p-4 border-2 border-green-600 rounded-md shadow-sm bg-gray-100 items-left">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center justify-center w-10 h-10 mb-auto bg-white rounded-full"><svg width="22"
        ...
        <div class="flex flex-col p-4 border-2 border-green-600 rounded-md shadow-sm bg-gray-100 items-left">
          <div class="flex items-center justify-between">
            ...
            <div class="flex items-center justify-center w-10 h-10 mb-auto bg-white rounded-full">
        ...
        <!-- Failed -->
        <div class="flex flex-col p-4 border-2 border-red-600 rounded-md shadow-sm bg-gray-100 items-left">
          <div class="flex items-center justify-between">
            <span class="text-3xl font-semibold">{{ totals.failed }}</span>
            <div class="flex items-center justify-center w-10 h-10 mb-auto bg-white rounded-full">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M1 1L13 13M1 13L13 1" stroke="#FF5757" stroke-width="2" stroke-linecap="round"
      ...
      <div class="p-4 overflow-x-auto max-h-[51vh] custom-scrollbar">
        <table class="w-full text-sm bg-white border border-gray-300 rounded-lg md:text-base">
          <thead>
            <tr class="font-semibold text-center text-gray-700 bg-gray-100">
              <th class="sticky top-0 p-2 border-b-2 md:p-4">Name</th>
              <th class="sticky top-0 p-2 border-b-2 md:p-4">Phone No</th>
              <th class="sticky top-0 p-2 border-b-2 md:p-4">Status</th>
              <th class="sticky top-0 p-2 border-b-2 md:p-4">Failure Reason</th>
            </tr>
          </thead>
          <tbody class="bg-white">
            <tr v-for="(contactReport, index) in broadcastReports" :key="contactReport.id">
              <td class="p-2 text-center border-[#ddd] md:p-4">{{ contactReport.contact_name }}</td>
              <td class="p-2 text-center border-[#ddd] md:p-4">{{ contactReport.phone_no }}</td>
              <td class="p-2 text-center border-[#ddd] md:p-4">{{ contactReport.status }}</td>
              <td class="p-2 text-center border-[#ddd] md:p-4">
                <div class="relative flex items-center justify-center">
                  ...
                  <div v-if="tooltipVisible === index"
                    class="absolute z-50 p-2 text-sm text-white bg-gray-700 rounded tooltip-container"
                    :style="tooltipStyles">
...
export default {
  ...
  <img src="${component.example.header_handle[0]}" alt="Description of image" 
    ...
    replacePlaceholders(bodyText, example) {
      if (example && example.length > 0) {
        example.forEach((param, index) => {
          ...
          bodyText = bodyText.replace(placeholder, param[0]);
        });
      }
      return bodyText;
    },
    ...
    async onTemplateSelect() {
      ...
      if (headerComponent) {
        ...
        if (this.imageUrl) {
          try {
            const token = localStorage.getItem('token');
            const downloadUrl = new URL(`${this.apiUrl}/download-media`);
    ...
    async uploadMedia() {
      if (!this.mediafile) return console.error("No file available for upload");
      const token = localStorage.getItem('token');
    ...
    async sendBroadcast() {
      ...
      const broadcastNameWithDate = `${this.broadcastName} - ${this.formatDateTime(new Date())}`;
      const token = localStorage.getItem('token');
      ...
      try {
        ...
        if (this.mediaId) requestBody.image_id = this.mediaId;
        if (this.bodyParameter) requestBody.body_parameters = this.bodyParameter;
    ...
    async fetchBroadcastReport(broadcast_id) {
      ...
      try {
        ...
        this.broadcastReports = result.map(report => ({ ...report }));
      } catch (error) {
    ...
    clearForm() {
      ...
      this.previewData = '';
      this.fetchContacts(); // Reset contacts list
    },
    ...
    async importCSV() {
      ...
      try {
        ...
        if (response.status >= 200 && response.status < 300) {
          ...
          const { contacts, duplicates } = response.data;
          this.contacts = [...contacts, ...duplicates];
  ...
  watch: {
    selectedContacts: function () {
      ...
      this.allSelected = this.contacts.length > 0 && this.selectedContacts.length === this.contacts.length;
    }
  }
};
</script>
